...
void c3DView::OnEventProc(const sf::Event &evt)
{
	...
	case sf::Event::KeyPressed:
		...
		case sf::Keyboard::Space:
		{
			...
			{
			case 0:
				m_physSync->SpawnBox(GetRenderer(), pos, Vector3::Ones * scale);
				break;
			case 1:
				m_physSync->SpawnSphere(GetRenderer(), pos, scale);
				break;
			case 2:
				m_physSync->SpawnCapsule(GetRenderer(), pos, scale, scale*2.f);
				break;
			}
			++idx;
			idx %= 3;
