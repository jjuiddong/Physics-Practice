...
// euler (roll/pitch/yaw) -> quaternion
//		roll:x-axis, pitch:z-axis, yaw:y-axis
// convert OpenGL space -> Direct space
// euler: degree angle
Quaternion cNode2::EulerToQuat2(const Vector3& euler)
{
	if (euler.IsEmpty())
		return Quaternion(); // zero rotation
	const Quaternion xRot(Vector3(1, 0, 0), ANGLE2RAD(-euler.x)); // roll
		...
		if (data->name == "link")
		{
			...
			for (auto& child : data->children)
			{
				if (child->name == "visual")
				{
					...
					vis->m_height = sdata.Get<float>(child, "height", 0.f);
					...
					if (geoType == "cylinder")
					{
						vis->m_radius = vis->m_dimension.x;
						vis->m_height = vis->m_dimension.y;
					}
					else if (geoType == "mesh")
					{
						vis->m_dimension = Vector3(0.3f, 0.3f, 0.3f);
					}
...
// convert directx space
bool rik::cUrdf3::ToDX()
{
	ConvertSpace();
	return true;
}

// convert opengl space
bool rik::cUrdf3::ToOpenGL()
{
	ConvertSpace();
	return true;
}

// convert space (DX <-> OpenGL)
bool rik::cUrdf3::ConvertSpace()
{
	Traverse([&](cNode2* node, const Matrix44& tm) {
		node->m_transform.pos = node->m_transform.pos.ToOpenGL();

		// tricky code: only once convoert availible
		// opengl -> dx -> opengl coverting error
		node->m_transform.rot = node->EulerToQuat2(node->m_euler);

		if (3 == node->m_nodeType) // visual
		{
			// nothing to do
		}
		else if (2 == node->m_nodeType) // joint
		{
			cJoint2* joint = dynamic_cast<cJoint2*>(node);
			if (joint)
			{
				joint->m_axis = joint->m_axis.ToOpenGL();
			}
		}

		return false; // all joint loop
		});
	return true;
}
